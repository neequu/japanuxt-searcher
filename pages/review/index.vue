...
<template>
  ...
  <section v-else class="mt-8 h-full flex flex-1 flex-col items-center md:place-content-center md:gap-4">
    ...
    <button v-if="dueWords.length" type="button" aria-label="review" class="group mx-auto inline-flex border border-accent rounded-xl px-5 py-2 text-accent outline-none transition-300 md:mx-0 md:mt-0 md:px-10 md:py-4 md:text-2xl hover:shadow-accent" @click="reviewSession = true">
